...
export default function SlideItem({ title, description, image }: SlideItemProps) {
  ...
  return (
    <View style={{ width }} className="items-center justify-center px-4">
      ...
      <Text style={{fontFamily:'Poppins'}} className="text-2xl font-semibold text-center mb-2 text-black">{title}</Text>
      <Text style={{fontFamily:'Poppins'}} className="text-base font-medium text-center  text-gray-500">{description}</Text>
    </View>
  );
}
